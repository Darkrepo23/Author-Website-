...
BOOKS_FOLDER = os.path.join(os.getcwd(), "books-pdf")

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
BOOKS_PDF_FOLDER = os.path.join(BASE_DIR, "books-pdf")
...
@app.route("/books-pdf/<path:filename>")
def get_book_file(filename):
    try:
        print("📂 Request for file:", filename)
        return send_from_directory(BOOKS_FOLDER, filename, as_attachment=False)
...
def get_admin_info():
    ...
    # Get admin by FullName = 'Administrator'
    cursor.execute("""
...
def update_admin():
    ...
    return jsonify({"message": "Administrator info updated successfully!"})
...
def serve_pdf(filename):
    return send_from_directory(BOOKS_PDF_FOLDER, filename)
...
def get_books():
    ...
    for row in cursor.fetchall():
        ...
        if book.get("pdf"):
            filename = os.path.basename(book["pdf"])  # get only the file name
            book["pdf"] = f"/books-pdf/{filename}"    # build URL for frontend
